...
class Scanner(QMainWindow):
    ...
    ----------
    name: str
        Name of devices scanned for for user interactions.
    ...
    def __init__(self, name, discover_funcs, device_gui, *args, **kwargs):
    ...
    def scan_ports(self):
        ...
        if self.scan.discover():
            ...
            for dev in self.scan:
                d = (dev.device, dev.model, dev.serial_number)
    ...
    def display(self):
        ...
        text += f'Scanning for {self.name}s ...<br/>'
        if len(self.devices) > 0:
            ...
            text += f'Please connect a {self.name} to an USB port.'
